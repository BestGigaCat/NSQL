...
class CustomSQLChain(Chain):
    """Chain for interacting with SQL Database.

    Example:
        .. code-block:: python

            from langchain_experimental.sql import CustomSQLChain
            from langchain.llms import OpenAI, SQLDatabase
            db = SQLDatabase(...)
            db_chain = CustomSQLChain.from_llm(OpenAI(), db)
    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    def raise_deprecation(cls, values: Dict) -> Dict:
        if "llm" in values:
            warnings.warn(
                "Directly instantiating an CustomSQLChain with an llm is deprecated. "
    ...
    def from_llm(
        ...
        """Create a CustomSQLChain from an LLM and a database connection.
...
class SQLDatabaseSequentialChain(Chain):
    ...
    def from_llm(
        ...
        sql_chain = CustomSQLChain.from_llm(llm, db, prompt=query_prompt, **kwargs)
        decider_chain = LLMChain(
